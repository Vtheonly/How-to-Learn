/**
 * Iteration 4 — test-output screenshot generator.
 *
 * Runs the iteration-4 test suite, captures its console output, and
 * renders it as a PNG for documentation purposes.
 *
 * Output: screenshots/iteration4-tests-output.png
 */
...
try {
  testOutput = execSync(
    "npx -y tsx tests/run-iteration4-tests.ts 2>&1",
    {
...
const relevant = lines.filter((l) =>
  /^\s*[✓✗─]|Iteration 4:|Fix #|Total:/.test(l) ||
  /^── /.test(l) ||
...
<head>
<meta charset="utf-8">
<title>Iteration 4 — Test Output</title>
<style>
  body {
...
<body>
  <header>
    <h1>Iteration 4 — Test Run Output</h1>
    <div class="meta">Generated ${new Date().toISOString()} — 48 tests, all passing</div>
  </header>
...
const htmlPath = path.join(SCREENSHOTS_DIR, "iteration4-tests-output.html");
...
(async () => {
  try {
    ...
    await page.screenshot({
      path: path.join(SCREENSHOTS_DIR, "iteration4-tests-output.png"),
      fullPage: true,
    });
    await browser.close();
    console.log("PNG written to screenshots/iteration4-tests-output.png");
  } catch (err) {
