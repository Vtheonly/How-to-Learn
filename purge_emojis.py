...
class EmojiPurgeService:
    ...
    def scan_and_purge(self, root_dir: str) -> dict:
        """
        Recursively scan root_dir for text files and purge emojis from them.
        Returns a summary dict with file counts and emoji counts.
        """
        summary = {
            'files_scanned': 0,
            'files_modified': 0,
        ...
        for dirpath, dirnames, filenames in os.walk(root_dir):
            ...
            for filename in filenames:
                ...
                try:
                    ...
                    cleaned = self.transform(content)
                    with open(filepath, 'w', encoding='utf-8') as f:
                        f.write(cleaned)

                    summary['files_modified'] += 1
                    summary['total_emojis_removed'] += emoji_count
                    print(f"  Purged {emoji_count} emoji(s) from: {filepath}")
...
def main():
    ...
    print(f"Scanning {root} for emojis...")
    service = EmojiPurgeService()
    summary = service.scan_and_purge(root)

    print("\n=== Summary ===")
    print(f"Files scanned:       {summary['files_scanned']}")
    print(f"Files modified:      {summary['files_modified']}")
    print(f"Total emojis removed: {summary['total_emojis_removed']}")
    if summary['errors']:
